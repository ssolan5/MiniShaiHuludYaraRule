...
This repo is for the `YARA` rules that I am writing, for helping detect the `npm` packages affected by the **Mini Shai Hulud** Supply Chain attack ( duration 29th April 2026 to 12th May 2026 ) in npm projects that list the compromised packages in their `package.json` or in the `npm` environment of a developer. The threat actor identified is **TeamPCP**.
